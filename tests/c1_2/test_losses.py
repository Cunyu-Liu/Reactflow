...
def _make_simple_batch(L: int = 6, *, batch: int = 2, seed: int = 0):
    ...
    # Random binary targets (symmetric).  Use maximum (not average) so the
    # symmetrized matrix remains binary {0, 1} — averaging would produce 0.5
    # values that break "perfect prediction" loss tests.
    targets = torch.randint(0, 2, (batch, L, L)).float()
    targets = torch.maximum(targets, targets.transpose(1, 2))
    for i in range(L):
        targets[:, i, i] = 0.0
    # Mask
...
class TestUnpairedBCELoss:
    def test_non_negative(self):
        logits, targets, mask = _make_simple_batch()
        B, L = logits.shape[:2]
        unpaired_logit = torch.randn(B, L)
    ...
    def test_gradient_flow(self):
        logits, targets, mask = _make_simple_batch()
        B, L = logits.shape[:2]
        unpaired_logit = torch.randn(B, L, requires_grad=True)
        loss = unpaired_bce_loss(unpaired_logit, targets, mask=mask)
        loss.backward()
