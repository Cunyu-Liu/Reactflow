...
def test_held_prediction_never_reads_mutant_outcome(monkeypatch) -> None:
    import torch
    ...
    for index in range(8):
        ...
        sequence = torch.zeros(length, 4)
        sequence[:, 0] = 1.0
        contexts[construct_id] = (
            sequence,
            torch.full((length,), float(index + 1)),
            torch.ones(length),
            torch.ones(length),
            torch.arange(length, dtype=torch.float32),
            torch.column_stack([torch.ones(length), torch.zeros(length)]),
        )
    ...
    np.testing.assert_array_equal(prediction["parent_point"], 0.0)
    np.testing.assert_array_equal(prediction["aligned_point"], 0.0)
    np.testing.assert_array_equal(prediction["shift17_point"], 0.0)
