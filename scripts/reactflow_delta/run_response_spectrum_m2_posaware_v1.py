#!/usr/bin/env python3
"""run_response_spectrum_m2_posaware_v1 — POSITION-AWARE residual response spectrum
on OpenKnot M2.

Method-level upgrade over run_response_spectrum_m2_v1:
  * v1 used ResidualSpectrumMLP (single shared output head -> all WINDOW positions).
  * v3 uses PositionAwareResidualMLP: shared trunk encodes the local-window features,
    then each window position has its OWN decoder head + a learned position embedding.

This is the "stronger per-position model" direction chosen to push WMAE skill beyond
the mu-ensemble residual MLP (~+8.9%).  Same protocol / exchangeable unit / residual
fail-safe / CUDA-only contract as v1.

Variant name: wmae_resid_posaware_spectrum (seeds 0..4), baseline wmed_spectrum (seed 0).
"""
...
import residual_spectrum_v3 as rsv3  # noqa: E402  (position-aware model)

MODEL_ID = "response_spectrum_m2_posaware_v1"
CALLER_VERSION = "m2_caller_v1"
MODEL_VARIANT = "wmae_resid_posaware_spectrum"
...
def main():
    ...
    print(f"[m2pa] GPU OK: cuda_visible={args.cuda_device} idx={gpu_index} "
    ...
    print(f"[m2pa] rows={dmeta['n_rows']} designs={dmeta['n_designs']} "
    ...
    print(f"[m2pa] changers with usable spectrum={len(spectra)}", flush=True)
    if not spectra:
        raise RuntimeError("no changers with usable spectra; STOP.")

    # ---- LOO over designs ----
    pred_path = out / "keyed_predictions_m2_posaware.jsonl"
    ...
    for fold, held in enumerate(resolved):
        ...
        for seed in SEEDS:
            model, tlog = rsv3.train_posaware(
                Xtr, Ytr, Wtr, prior, epochs=args.epochs, bs=args.bs, lr=args.lr,
                resid_pen=args.resid_pen, hidden=args.hidden, head_hidden=args.head_hidden,
                dropout=args.dropout, seed=seed, device=device)
            delta = rsv3.predict_delta(model, Xte, device)
    ...
    (out / "posaware_training_log.json").write_text(
        json.dumps(global_log, indent=2, sort_keys=True), encoding="utf-8")

    dhash = sha256_file(pred_path)
    manifest = {
        "schema": "reactflow_delta.response_spectrum.m2_posaware.v1",
        ...
        "experiment": "position_aware_residual_full_spectrum_response_m2",
        ...
        "source_hashes": {
            "run_response_spectrum_m2_posaware_v1.py": sha256_file(Path(__file__)),
            ...
            "residual_spectrum_v3.py": sha256_file(Path(__file__).resolve().parent / "residual_spectrum_v3.py"),
    ...
    (out / "response_spectrum_m2_posaware_manifest.json").write_text(
