    ...
    for p in range(1, n_puzzles + 1):
        ...
        for method in methods:
            rec = {"id": f"{puzzle}_{method}_wt", "sequence": seq,
                   ...
                   "puzzle": puzzle, "method": method, "sub_start": sub[0],
                   "sub_end": sub[1], "target_structure": "", "mutA": 0,
                   "M2_structure": "AAAA"}
            for i in range(1, seq_len + 1):
                rec[f"reactivity_{i:04d}"] = float((i % 5)) / 4 + 0.1
                rec[f"reactivity_error_{i:04d}"] = 0.1
            rows.append(rec)
            for pos in range(sub[0], sub[1]):
                ref = seq[pos]
                for alt in [b for b in bases if b != ref]:
                    m = dict(rec)
                    m["id"] = f"{puzzle}_{method}_mm_{pos}_{ref}_{alt}"
                    m["sequence"] = seq[:pos] + alt + seq[pos + 1:]
                    m["mutA"] = pos - sub[0] + 1
