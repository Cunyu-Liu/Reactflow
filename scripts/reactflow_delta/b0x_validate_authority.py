#!/usr/bin/env python3
"""Fail-closed B0-X authority preflight; performs no network or data access.

Mirrors the PH0-X validator for the B0-X Strong Baseline Qualification
authority epoch 10.  Verifies only B0-X is runnable, D0-X/D1-X/D2-X/PH0-X are
PASS, the B0-X amendment/approval hashes are bound and intact, and the
...
def validate(root: Path, *, staging: bool = False) -> dict[str, Any]:
    ...
    check(authority.get("current_phase") == "B0-X", "current phase is not B0-X")
    check(auth.get("allowed_phases") == ["D0-X", "D1-X", "D2-X", "PH0-X", "B0-X"],
          "allowed phases mismatch")
    check(auth.get("runnable_phases") == ["B0-X"], "runnable phases must equal [B0-X]")
    ...
    # B0-X specifically authorizes baseline/paired-model training within the capacity ladder.
    check(auth.get("training_allowed") is True, "training (capacity-ladder baselines) must be allowed")
    ...
    check(bindings.get("b0x_run_id") == "b0x_strong_baseline_20260804_v1", "b0x_run_id mismatch")

    phases = {row.get("phase_id"): row for row in active.get("phase_graph", [])}
    for pid in ("RECOVERY_CONTRACT_REWRITE", "PILOT_CLOSURE", "D0-X", "D1-X", "D2-X", "PH0-X"):
        check(phases.get(pid, {}).get("gate_result") == "PASS", f"{pid} not PASS")
    check(phases.get("B0-X", {}).get("execution_authorized") is True, "B0-X execution not authorized")
    ...
    return {
        "schema_version": "reactflow_delta.b0x_authority_preflight.v1",
        "status": "PASS",
