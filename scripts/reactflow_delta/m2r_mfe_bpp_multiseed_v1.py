#!/usr/bin/env python3
"""m2r_mfe_bpp_multiseed_v1.py — multi-seed strong 3-way + MFE + BPP features.

Full design-level audit of the partition-function (BPP) feature extension on
top of the MFE thermodynamic modality, using the headline method
(K=5 L1/L2 OOF averaging + Ridge, 0.6/0.3/0.1):

  * base     = v1+v2+transfer (258)          -> multi-seed 3-way  [+29.22%]
  * mfe      = base + MFE (292)              -> multi-seed 3-way  [+30.33%]
  * mfe_bpp  = base + MFE + BPP (309)        -> multi-seed 3-way  [this run]

Compares mfe_bpp multi-seed vs the SAVED MFE multi-seed OOF
(m2r_mfe_multiseed_20260818/m2r_mfe_multiseed_oof.npz blend_K) for a leak-free,
LOO-exclusion + permutation-significant marginal-gain statement.
"""
...
def run_mfe_bpp_multiseed(X, y, keys, args) -> dict:
    out = Path(args.out)
    ...
    for k, seed in enumerate(SEEDS):
        ...
        l1_seeds.append(p)
        print(f"[bppms] L1 seed {seed} skill={_skill(_mae(y, p), mae_bl):+.4f} "
              f"wall={time.time()-t0:.0f}s", flush=True)
        p = _loo_lgb_seed(X, y, keys, des_list, "regression", seed)
        l2_seeds.append(p)
        print(f"[bppms] L2 seed {seed} skill={_skill(_mae(y, p), mae_bl):+.4f} "
              f"wall={time.time()-t0:.0f}s", flush=True)
    ...
    # ---- compare vs saved MFE multi-seed baseline (blend_K) ----
    ...
    if len(base_blend) != len(y) or np.abs(base_y - y).max() > 1e-9:
        raise SystemExit("[bppms] baseline npz y mismatch")
    base_skill = _skill(_mae(y, base_blend), mae_bl)

    # LOO-exclusion gain (mfe+bpp multi-seed vs mfe multi-seed)
    gains = []
    for held in des_list:
        m = keys != held
        if m.sum() < 10:
    ...
    report = {
        "schema": "reactflow_delta.m2r_mfe_bpp_multiseed.v1",
        ...
        "baseline_mae": mae_bl,
        "results": {
            "mfe_bpp_single_seed_3way": {"mae": _mae(y, blend_1),
                                         "skill": _skill(_mae(y, blend_1), mae_bl),
                                         "r2": _r2(y, blend_1)},
            "mfe_bpp_multiseed_3way": {"mae": _mae(y, blend_K),
                                       "skill": _skill(_mae(y, blend_K), mae_bl),
                                       "r2": _r2(y, blend_K)},
            "mfe_multiseed_3way": {"mae": _mae(y, base_blend),
                                   "skill": base_skill,
                                   "r2": _r2(y, base_blend)},
        },
        "bpp_gain_vs_mfe": {
    ...
    (out / "m2r_mfe_bpp_multiseed_report.json").write_text(
        json.dumps(report, indent=2, sort_keys=True), encoding="utf-8")
    np.savez(out / "m2r_mfe_bpp_multiseed_oof.npz",
             blend_1=blend_1, blend_K=blend_K, y=y, keys=keys)
    g = report["bpp_gain_vs_mfe"]
    print(f"\n=== M2R MFE+BPP multi-seed (K={len(SEEDS)}) design-level LOO ===")
    print(f"MFE multi-seed     : skill={base_skill:+.4f} "
          f"R2={_r2(y, base_blend):.4f}")
    print(f"MFE+BPP single-seed: skill={report['results']['mfe_bpp_single_seed_3way']['skill']:+.4f} "
          f"R2={report['results']['mfe_bpp_single_seed_3way']['r2']:.4f}")
    print(f"MFE+BPP multi-seed : skill={report['results']['mfe_bpp_multiseed_3way']['skill']:+.4f} "
          f"R2={report['results']['mfe_bpp_multiseed_3way']['r2']:.4f}")
    ...
    print(f"wall={report['wall_seconds']:.0f}s DONE -> "
          f"{out / 'm2r_mfe_bpp_multiseed_report.json'}")
    return report
...
def main():
    ...
    ap.add_argument("--base-npz", required=True,
                    help="m2r_mfe_multiseed_oof.npz (MFE baseline blend_K)")
    ...
    X = np.concatenate([X1, X2, X_tr, XM, XB], axis=1)
    print(f"[bppms] n={len(y)} X={X.shape} K={len(SEEDS)}", flush=True)

    run_mfe_bpp_multiseed(X, y, keys, args)
    return 0
