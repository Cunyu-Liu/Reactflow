#!/usr/bin/env python3
"""Fail-closed D0-X authority preflight; performs no network or data access."""
...
def validate(root: Path, *, staging: bool = False) -> dict[str, Any]:
    ...
    check(authority.get("current_phase") == "D0-X", "current phase is not D0-X")
    check(auth.get("allowed_phases") == ["D0-X"], "allowed phases must equal [D0-X]")
    check(auth.get("runnable_phases") == ["D0-X"], "runnable phases must equal [D0-X]")
    ...
    check(phases.get("D0-X", {}).get("execution_authorized") is True, "D0-X execution not authorized")
    ...
    return {
        "schema_version": "reactflow_delta.d0x_authority_preflight.v1",
        "status": "PASS",
