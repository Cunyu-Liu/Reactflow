#!/usr/bin/env python3
"""Score the frozen V12 five-seed mixture and all constituent seeds."""
...
from scripts.reactflow_delta.assemble_model_rescue_v12_formal import (
...
from scripts.reactflow_delta.score_model_rescue_v12 import (
...
SCHEMA = "reactflow_delta.model_rescue_v12_formal_score.v1"
...
def assert_score_authority(repo_root: Path) -> None:
    ...
    if active["authority"]["current_phase"] != "V12M4" or active.get(
        "runnable_phases"
    ) != ["V12M4"]:
        raise RuntimeError("V12 formal scorer is closed outside V12M4")
    if active.get("training_allowed") is not False or active.get(
        "candidate_model_training_allowed"
    ) is not False:
        raise RuntimeError("V12M4 training must be closed before formal scoring")
    if active.get("held_score_read_allowed") != (
        "V12_FORMAL_COMPLETE_ASSEMBLY_SCORE_ONCE_ONLY"
    ):
        raise RuntimeError("V12M4 complete formal score access is closed")
    if active.get("partial_fold_score_read_allowed") is not False:
        raise RuntimeError("V12M4 partial score access must remain closed")
    if active.get("new_external_outcome_access_allowed") is not False:
        raise RuntimeError("V12M4 external outcomes must remain locked")
...
def score_formal(
    ...
    if assembly.get("schema_version") != ASSEMBLY_SCHEMA or assembly.get("status") != (
        "V12M4_FIVE_SEED_PREDICTION_ONLY_ASSEMBLY_PASS"
    ):
        raise ValueError("V12 formal scorer requires complete assembly")
    if merged.get("schema_version") != MERGED_SCHEMA or merged.get("status") != (
        "V12M4_COMPLETE_UNSCORED_MERGE_PASS"
    ):
        raise ValueError("V12 formal scorer requires complete fold-seed merge")
    if not merged_integrity_pass(merged.get("merge_integrity", {})):
        raise ValueError("V12 formal merged integrity is not qualified")
    if not (
        ...
        and assembly.get("partial_score_inspected") is False
        and assembly.get("external_outcome_accessed") is False
    ):
        raise ValueError("V12 formal assembly violates the score-blind protocol")
    if tic2a.get("schema_version") != TIC2A_MERGED_SCHEMA or tic2a.get("status") != (
        "TIC2A_COMPLETE_UNSCORED_MERGE_PASS"
    ):
        raise ValueError("V12 formal scorer requires corrected TIC2A merge")
    ...
    if (
        ...
        or set(source_rows) != expected_sources
        or sorted(tic_rows) != list(range(20))
    ):
        raise ValueError("V12 formal score universes are incomplete")
    univ = M2Universe(m2_csv)
    identity = univ.build()
    if identity.get("canonical_mutant_full_profile_identity") != (
        "EXACT_PUZZLE_METHOD_MUTATION"
    ):
        raise RuntimeError("V12 formal scorer requires exact target identity")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.out_json.exists():
        raise FileExistsError(f"V12 refuses to overwrite its one formal score: {args.out_json}")
    result = score_formal(
        json.loads(args.assembly_json.read_text()),
