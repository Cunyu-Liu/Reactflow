...
preflight_gpu() {
  ...
  if CUDA_VISIBLE_DEVICES="${gpu}" "${python_bin}" -c \
    ...
    status=$?
    printf 'V14M4 CUDA preflight failed: cuda_visible_devices_value=%s logical_device=cuda:0 status=%s\n' \
      "${gpu}" "${status}" >&2
    return "${status}"
  fi
}
...
if ((${#tasks[@]} > 0)); then
  if ((${#gpus[@]} == 0)); then
    printf 'V14M4 has missing tasks but no CUDA_VISIBLE_DEVICES value was supplied; logical_device=cuda:0\n' >&2
    exit 2
  fi
  preflight_failed=0
  for gpu in "${gpus[@]}"; do
...
while ((${#active_pids[@]} > 0)); do
  ...
  if ((task_status != 0)); then
    failed=1
    printf 'V14M4 task %s with cuda_visible_devices_value=%s failed with status %s\n' \
...
if [[ "${failed}" -ne 0 ]]; then
  printf 'one or more V14M4 prediction tasks failed; merge and assembly were not run\n' >&2
  exit 1
fi
