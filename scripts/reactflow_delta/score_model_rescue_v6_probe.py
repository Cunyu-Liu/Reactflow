#!/usr/bin/env python3
"""Join targets and score V6M2 only after the complete prediction merge."""
...
from scripts.reactflow_delta.merge_model_rescue_v6_probe import SCHEMA as MERGED_SCHEMA
from scripts.reactflow_delta.run_model_rescue_v6_probe import PREDICTION_SCHEMA
from scripts.reactflow_delta.run_p2_v3 import _bio_key
from scripts.reactflow_delta.split_v4_lopo_puzzle import build_split_v4


SCHEMA = "reactflow_delta.model_rescue_v6_probe_score.v1"
PREDICTION_FIELDS = (
...
def assert_score_authority(repo_root: Path) -> None:
    ...
    if active["authority"]["current_phase"] != "V6M2":
        raise RuntimeError("v6 probe scorer is closed outside active V6M2")
    if active.get("held_score_read_allowed") is not True:
        raise RuntimeError("complete V6M2 score access has not been opened")
    if active.get("partial_fold_score_read_allowed") is not False:
        raise RuntimeError("partial V6M2 scores must remain prohibited")
    if active.get("new_external_outcome_access_allowed") is not False:
        raise RuntimeError("v6 scorer requires external outcomes locked")
...
def _load_prediction(path: Path, outer_fold: int) -> dict[str, np.ndarray]:
    ...
    if str(prediction["schema_version"].item()) != PREDICTION_SCHEMA:
        raise ValueError(f"invalid v6 prediction schema in {path}")
    keys = prediction["keys"]
    if not np.array_equal(keys, prediction["biological_scoring_key"]):
        raise ValueError(f"v6 prediction keys disagree in {path}")
    if len(set(map(str, keys))) != len(keys):
        raise ValueError(f"v6 prediction keys are not unique in {path}")
    if set(map(int, prediction["outer_fold"])) != {outer_fold}:
        raise ValueError(f"v6 prediction outer fold mismatch in {path}")
    for field in PREDICTION_FIELDS:
        if prediction[field].shape != (len(keys),) or not np.isfinite(
            prediction[field]
        ).all():
            raise ValueError(f"v6 prediction {field} is invalid in {path}")
    forbidden = {
        "target",
        "target_error",
        "target_mask",
    ...
    if not forbidden.isdisjoint(prediction):
        raise ValueError(f"v6 prediction contains target-side fields in {path}")
    return prediction
...
def _puzzle_macro(losses: dict[str, float]) -> float:
    ...
    for key, loss in losses.items():
        ...
        raw = key.split("|")
        if len(raw) < 7:
            raise ValueError(f"invalid V6M2 biological scoring key {key}")
    ...
    if len(puzzles) != 1 or not mutant_losses:
        raise ValueError("one V6M2 fold must contain exactly one scored held puzzle")
    ...
    for record in held_records:
        ...
        for position in np.flatnonzero(qualified):
            ...
            if key not in key_to_index:
                raise ValueError("qualified V6M2 target is missing from prediction ledger")
...
def score_complete_merged(merged: dict[str, Any], m2_csv: Path) -> dict[str, Any]:
    if merged.get("schema_version") != MERGED_SCHEMA:
        raise ValueError("v6 scorer requires the complete merged schema")
    integrity = merged.get("merge_integrity", {})
    if integrity.get("complete_fold_universe") is not True:
        raise ValueError("v6 scorer cannot access an incomplete fold universe")
    if integrity.get("v5_baseline_replay_all_folds") is not True:
        raise ValueError("v6 scorer requires v5 baseline replay on every fold")
