#!/usr/bin/env python3
"""D0-X formal terminal closure finalizer.

Re-verifies the D0-X authority, runs the committed D0-X unit tests, runs the
inventory audit, binds the manual audit, then atomically publishes a terminal
manifest, checksum ledger, and terminal sentinel outside Git.  Only the
D0-X candidate inventory is being closed; no exact-pair, eligibility, Tier,
...
D0X_FINALIZER_SCHEMA = "reactflow_delta.d0x_finalizer.v1"
D0X_AUTHORITY_VALIDATOR = "scripts/reactflow_delta/d0x_validate_authority.py"
D0X_AUDIT_SCRIPT = "scripts/reactflow_delta/d0x_audit_inventory.py"
D0X_PARSE_SCRIPT = "scripts/reactflow_delta/d0x_parse_all_rdat.py"
D0X_TESTS = [
    "tests/reactflow_delta/test_d0x_contract_parser.py",
    "tests/reactflow_delta/test_d0x_rmdb_snapshot.py",
]
MANUAL_AUDIT_PATH = "docs/audits/reactflow_delta_d0x_manual_audit_20260803.md"
...
def run_finalizer(
    ...
    try:
        ...
        auth_run = subprocess.run(
            [sys.executable, D0X_AUTHORITY_VALIDATOR],
            cwd=repo_root,
            text=True,
        ...
        if auth_run.returncode != 0:
            raise RuntimeError("D0-X authority verification failed")
        ...
        audit_run = subprocess.run(
            [
                ...
                str(raw_dir),
                "--output-json",
                str(staging / "inventory_audit.json"),
            ],
            cwd=repo_root,
            text=True,
            stdout=subprocess.PIPE,
        ...
        audit_log = staging / "inventory_audit_run.txt"
        audit_log.write_text(audit_run.stdout, encoding="utf-8")
        if audit_run.returncode != 0:
            raise RuntimeError("inventory audit failed (not all assets parsed or silent drop)")

        # ---- automated tests ----
        test_run = subprocess.run(
            [sys.executable, "-m", "unittest", *D0X_TESTS, "-v"],
            cwd=repo_root,
            text=True,
            stdout=subprocess.PIPE,
            stderr=subprocess.STDOUT,
        ...
        automated = {
            ...
            "command": ["python3", "-m", "unittest", *D0X_TESTS, "-v"],
            "tool_versions": {
        ...
        if test_run.returncode != 0:
            raise RuntimeError("D0-X automated tests failed")
        ...
        manual = {
            ...
            "scope": [
                "per_accession_disposition_100_percent",
                "parser_coverage_full",
                "field_retention_presence",
                "silent_drop_zero",
                "raw_dir_sidecar_headers_documented",
                "requalification_all_verified",
            ],
            "sample_mode": "FULL_D0X_INVENTORY_AUDIT",
        ...
        inventory = {
            "schema_version": "reactflow_delta.d0x_artifact_inventory.v1",
            "execution_source_commit": expected_source_commit,
            "files": [],
            "external_inputs": [
                {
                    "path": str(requalify_ledger),
                    "sha256": sha256_file(requalify_ledger),
                },
                {
                    "path": str(inventory_jsonl),
                    "sha256": sha256_file(inventory_jsonl),
                },
                {
        ...
        # ---- terminal manifest ----
        terminal_manifest = {
            "schema_version": "reactflow_delta.d0x_terminal.v1",
            ...
            "authority": {
                "validator_path": D0X_AUTHORITY_VALIDATOR,
                "verification_report": str(output_root / "authority_verification.txt"),
            },
            "automated_tests": automated,
            "manual_audit": manual,
            "inventory_audit": {
                "path": str(output_root / "inventory_audit.json"),
                "sha256": sha256_file(staging / "inventory_audit.json"),
            },
            "finalizer": {
                "schema_version": D0X_FINALIZER_SCHEMA,
                "path": "scripts/reactflow_delta/d0x_finalize_inventory.py",
            ...
            "terminal_sentinel": {
                "path": str(output_root / "D0X_CLOSED.yaml"),
                "status": "WRITTEN_AFTER_LEDGER",
            },
            "scientific_boundary": (
                "D0-X candidate inventory closed; no exact pair count, eligibility, "
                "Tier, split, training, or scientific claim. D1-X not started."
            ),
            "terminal_route": "STOP_AWAIT_D1X_AUTHORITY_AMENDMENT",
        }
        ...
        ledger_members = sorted(
            ...
            if path.name not in {"SHA256SUMS", "D0X_CLOSED.yaml"}
        )
        ledger_text = "".join(
        ...
        sentinel = {
            "schema_version": "reactflow_delta.d0x_terminal_sentinel.v1",
        ...
        write_yaml(staging / "D0X_CLOSED.yaml", sentinel)
        ...
        return {
            ...
            "terminal_sentinel_sha256": sha256_file(output_root / "D0X_CLOSED.yaml"),
            "execution_source_commit": expected_source_commit,
        }
...
def main() -> int:
    ...
    parser.add_argument("--requalify-ledger", type=Path, required=True)
    args = parser.parse_args()
    result = run_finalizer(
        args.repo_root.resolve(),
        args.output_root,
        args.expected_source_commit,
        args.inventory_jsonl.resolve(),
        args.summary_json.resolve(),
        args.asset_manifest.resolve(),
        args.raw_dir.resolve(),
        args.requalify_ledger.resolve(),
    )
