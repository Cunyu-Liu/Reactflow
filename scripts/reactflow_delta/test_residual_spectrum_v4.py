...
@pytest.mark.skipif(not torch.cuda.is_available(), reason="fast path is CUDA-only")
def test_fast_path_matches_eager_training():
    """fast=True (torch.compile + GPU-accumulated stats) must produce a model
    that is functionally equivalent to the eager path: same train MAE direction
    vs prior, and near-identical final predictions on held-out data."""
    pos, glob, Y, Wm = _mk_data(n=512, seed=7)
    prior, _ = rv2.per_position_prior(Y, Wm)
    dev = torch.device("cuda")
    m_eager, log_eager = rv4.train_posaware_attn2(
        pos, glob, Y, Wm, prior, POS_DIM, TAIL_DIM, W,
        epochs=6, bs=128, lr=1e-3, resid_pen=1e-3,
        hidden=64, head_hidden=16, nhead=4, nlayers=1, dropout=0.1,
        seed=0, device=dev, fast=False)
    m_fast, log_fast = rv4.train_posaware_attn2(
        pos, glob, Y, Wm, prior, POS_DIM, TAIL_DIM, W,
        epochs=6, bs=128, lr=1e-3, resid_pen=1e-3,
        hidden=64, head_hidden=16, nhead=4, nlayers=1, dropout=0.1,
        seed=0, device=dev, fast=True)
    # both beat the prior on train MAE (residual learning works in both paths)
    assert log_eager["final"]["mae_model_train"] < log_eager["final"]["mae_prior_train"]
    assert log_fast["final"]["mae_model_train"] < log_fast["final"]["mae_prior_train"]
    # held-out predictions must agree to tight tolerance (same training semantics)
    pte = np.random.default_rng(11).normal(size=(16, W, POS_DIM)).astype(np.float32)
    gte = np.random.default_rng(12).normal(size=(16, TAIL_DIM)).astype(np.float32)
    de = rv4.predict_posaware_attn(m_eager, pte, gte, dev)
    df = rv4.predict_posaware_attn(m_fast, pte, gte, dev)
    rel = np.max(np.abs(de - df)) / (np.max(np.abs(de)) + 1e-8)
    assert rel < 0.05, f"fast/eager prediction drift rel={rel:.4f}"
