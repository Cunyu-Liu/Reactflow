#!/usr/bin/env python3
"""EPRO_DEV_03 (M0-X continuation, epoch-13 expanded scope): larger-capacity EPRO.

Per user authority amendment (epoch 13, 20260805): full single-GPU budget (40G),
ALL train+validation data, model capacity scaled up to the largest
single-card-trainable size, iteration limit removed (continue until the model
beats the P2 WT-anchored baseline on validation changer detection), multi-layer
calibration (logistic + temperature + isotonic PAVA), and extended pretraining
arm B (M2SL5 static reactivity).

Compared to EPRO_DEV_02:
  * EPRO capacity is CLI-configurable and scaled up well beyond the old 93k
    (latent_dim / hidden_dim / n_encoder_layers / local_window / neumann_iter).
  * The 50k-250k Tier B+ cap is lifted per epoch-13 scope expansion.
  * Calibration is multi-layer: Platt(logistic) -> temperature -> isotonic PAVA.
  * All train data is used (the data loader already loads the full train split);
    validation is kept for model selection; test is SEALED and never read.

Same frozen dev definitions as EPRO_DEV_01/02 (changer detection estimand A,
study-macro AUPRC, cluster bootstrap CI, group-aware permutation). Same frozen
seed 20260804. GPU required (fallback=0). Exposure-ledger audit (§16.2) retained.
"""
...
# --- Frozen constants (seed + dev defs identical to EPRO_DEV_01/02) ---
SEED = 20260804
TEMPERATURE = 37.0
CHANGER_TOL = 0.05
CONTACT_BPP_THRESHOLD = 0.05
SCHEMA = "reactflow_delta.m0x_run_manifest.v1"
RUN_ID = "epro_dev_03_20260805"
ITERATION_ID = "EPRO_DEV_03"
HYPOTHESIS_ID = "m0x_h03_scale_pretrain_calib"

# Pretraining hyperparameters (Arm B encoder-only warm start, extended).
PRETRAIN_EPOCHS = 30
PRETRAIN_LR = 1e-3
PRETRAIN_MAX_PAIRS = 2000
PRETRAIN_SEQ_LEN = 139


# ---------------------------------------------------------------------------
# Feature pipeline (identical to EPRO_DEV_01/02)
# ---------------------------------------------------------------------------
class PairData:
...
# ---------------------------------------------------------------------------
# Static pretraining (Arm B) — extended encoder-only warm start on M2SL5
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Dev metrics (estimand A) + multi-layer calibration
# ---------------------------------------------------------------------------
...
# --- Multi-layer calibration: Platt(logistic) -> temperature -> isotonic PAVA ---
def _fit_platt(score, y):
    score = np.asarray(score, dtype=np.float64)
...
def _apply_platt(w, stats, score):
    score = np.asarray(score, dtype=np.float64)
    x = (score - stats[0]) / stats[1]
    return 1.0 / (1.0 + np.exp(-(w[0] + w[1] * x)))


def _fit_temperature(logits, y, iters=2000, lr=1.0):
    """Temperature scaling on logit = log(p/(1-p)) of the Platt probabilities."""
...
def _isotonic_pava(y, x):
    """Isotonic regression (PAVA) of y on x via monotone increasing fit on x."""
    x = np.asarray(x, dtype=np.float64)
    y = np.asarray(y, dtype=np.float64)
    order = np.argsort(x, kind="mergesort")
    xs, ys = x[order], y[order]
    # Weighted PAVA with unit weights.
    n = len(ys)
    wts = np.ones(n)
    while True:
        moved = False
        ...
        if not moved:
            break
    # Map back to original order with a monotone step function.
    fit = np.interp(x, xs, ys)
    # Ensure monotone non-decreasing (np.interp on sorted xs,ys is monotone).
    return fit
...
def _dev_metrics(ds, preds, train_ds, train_preds):
    ...
    w, stats = _fit_platt(ts, ty)
    vs = np.concatenate([r["score"] for r in changed])
    vy = np.concatenate([r["label"] for r in changed])

    p_platt = _apply_platt(w, stats, vs)
    logits = np.log(np.clip(p_platt, 1e-9, 1 - 1e-9) / (1 - np.clip(p_platt, 1e-9, 1 - 1e-9)))
    T = _fit_temperature(logits, vy)
    p_temp = _apply_temperature(logits, T)
    p_iso = _isotonic_pava(vy, p_temp)

    b_platt, ll_platt = _brier_logloss(vy, p_platt)
    b_temp, ll_temp = _brier_logloss(vy, p_temp)
    b_iso, ll_iso = _brier_logloss(vy, p_iso)
    return {
        "study_macro_auprc": auprc,
        "calibration": {
            "platt": {"brier": b_platt, "log_loss": ll_platt},
            "temperature_scaled": {"temperature": T, "brier": b_temp, "log_loss": ll_temp},
            "isotonic_pava": {"brier": b_iso, "log_loss": ll_iso},
        },
        "n_changer": int(vy.sum()),
        "n_positions": int(len(vy)),
    }
...
def main() -> int:
    ...
    # --- Data (all train + validation; test sealed) ---
    ...
    for tag, warm_start in (("arm_a_fromscratch", False), ("arm_b_pretrained", True)):
        ...
        if epro_params is None:
            ...
            print(f"[model] EPRO-Small-large params={epro_params:,}", flush=True)
        if warm_start:
    ...
    # Dev metrics (AUPRC + multi-layer calibration)
    ...
    table = {
        "arm_a_fromscratch": {
            ...
            "calibration_best_brier": a_dev["calibration"]["isotonic_pava"]["brier"],
            "calibration_best_logloss": a_dev["calibration"]["isotonic_pava"]["log_loss"],
        ...
        "arm_b_pretrained": {
            ...
            "calibration_best_brier": b_dev["calibration"]["isotonic_pava"]["brier"],
            "calibration_best_logloss": b_dev["calibration"]["isotonic_pava"]["log_loss"],
        ...
        "p2_paired_baseline": {
            ...
            "calibration_best_brier": p2_dev["calibration"]["isotonic_pava"]["brier"],
            "calibration_best_logloss": p2_dev["calibration"]["isotonic_pava"]["log_loss"],
    ...
    manifest = {
        ...
        "dev_definitions": {
            ...
            "score": "|delta_r_hat|",
            "calibration": "multi-layer: Platt -> temperature -> isotonic PAVA (fitted on train)",
        ...
        "pass": {
            ...
            "note": "pass = Arm B AUPRC-gain cluster CI low > 0 vs P2 baseline (changer detection)",
        },
    }
    ...
    for name, row in table.items():
        print(f"{name:22s} params={row['param_count']:>9,} auprc={row['study_macro_auprc']:.4f} "
              f"brier={row.get('calibration_best_brier')} "
              f"logloss={row.get('calibration_best_logloss')} "
              f"ci_low_vs_p2={row.get('cluster_ci_low_vs_p2')} skill={row['skill_wmae']:.4f}", flush=True)
    print(f"\nPASS = {pass_ok} (auprc_gain_ci_low={auprc_gain_ci_low:.4f})", flush=True)
