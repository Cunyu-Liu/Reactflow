...
out=/mnt/cunyuliu/reactflow_delta_model_rescue_v11/v11m3_screen_seed0
...
if [[ "${failed}" -ne 0 ]]; then
  printf 'one or more V11M3 prediction workers failed\n' >&2
  exit 1
fi
...
"${python_bin}" -m scripts.reactflow_delta.merge_model_rescue_v11 \
  --input-dir "${out}" \
  --phase V11M3 \
  --out-json "${out}/v11m3_complete_unscored_merge.json"
touch "${out}/v11m3_complete_prediction_only_merge_pass"
