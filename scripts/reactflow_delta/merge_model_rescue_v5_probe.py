#!/usr/bin/env python3
"""Merge the complete prediction-only V5M2 fold universe without scoring."""
...
FOLD_SCHEMA = "reactflow_delta.model_rescue_v5_probe_fold.v1"
SCHEMA = "reactflow_delta.model_rescue_v5_probe_merged.v1"
...
def _validate_prediction(path: Path, fold: int, expected_rows: int) -> None:
    with np.load(path, allow_pickle=True) as handle:
        ...
        if not required.issubset(fields):
            raise ValueError(f"V5M2 prediction fields are incomplete in {path}")
        forbidden = {"target", "target_error", "target_mask", "score"}
        if not forbidden.isdisjoint(fields):
            raise ValueError(f"V5M2 prediction contains target-side fields in {path}")
        if str(handle["schema_version"].item()) != PREDICTION_SCHEMA:
            raise ValueError(f"invalid V5M2 prediction schema in {path}")
        keys = handle["keys"]
        if len(keys) != expected_rows:
            raise ValueError(f"V5M2 prediction row count differs in {path}")
        if not np.array_equal(keys, handle["biological_scoring_key"]):
            raise ValueError(f"V5M2 biological keys differ in {path}")
        if len(set(map(str, keys))) != len(keys):
            raise ValueError(f"V5M2 biological keys are duplicated in {path}")
        if set(map(int, handle["outer_fold"])) != {fold}:
            raise ValueError(f"V5M2 outer fold differs in {path}")
        for name in (
            "baseline_signed_delta",
            "baseline_absolute_delta",
            "candidate_signed_delta",
            ...
            if handle[name].shape != (len(keys),) or not np.isfinite(handle[name]).all():
                raise ValueError(f"V5M2 prediction {name} is invalid in {path}")


def merge_folds(input_dir: Path) -> dict:
    expected = set(range(20))
    paths = sorted(input_dir.glob("v5_probe_fold_result_fold*.json"))
    rows = []
    seen = set()
    for path in paths:
        row = json.loads(path.read_text(encoding="utf-8"))
        if row.get("schema_version") != FOLD_SCHEMA or row.get("phase") != "V5M2":
            raise ValueError(f"invalid V5M2 fold result {path}")
        fold = int(row["outer_fold"])
        if fold in seen:
            raise ValueError(f"duplicate V5M2 fold {fold}")
        seen.add(fold)
        ...
        for key in ("prediction_artifact", "model_artifact"):
        ...
        rows.append(row)
    complete = seen == expected and len(rows) == 20
    if not complete:
        raise ValueError(f"V5M2 fold universe incomplete: found {sorted(seen)}")
    rows.sort(key=lambda row: int(row["outer_fold"]))
    return {
        "schema_version": SCHEMA,
        "phase": "V5M2",
        "status": "V5M2_COMPLETE_UNSCORED_MERGE_PASS",
        "folds": rows,
        "merge_integrity": {
