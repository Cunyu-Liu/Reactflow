#!/usr/bin/env python3
"""Score the frozen V13 five-seed mixture and constituent seeds."""
...
from scripts.reactflow_delta.assemble_model_rescue_v13_formal import (
...
SCHEMA = "reactflow_delta.model_rescue_v13_formal_score.v1"
...
def assert_score_authority(repo_root: Path) -> None:
    ...
    if active["authority"]["current_phase"] != "V13M4" or active.get(
        "runnable_phases"
    ) != ["V13M4"]:
        raise RuntimeError("V13 formal scorer is closed outside V13M4")
    if active.get("training_allowed") is not False or active.get(
        "candidate_model_training_allowed"
    ) is not False:
        raise RuntimeError("V13 formal training must be closed before scoring")
    if active.get("held_score_read_allowed") != "V13_FORMAL_COMPLETE_SCORE_ONCE_ONLY":
        raise RuntimeError("V13 formal score authority is closed")
    if active.get("partial_fold_score_read_allowed") is not False or active.get(
        "new_external_outcome_access_allowed"
    ) is not False:
        raise RuntimeError("V13 formal score boundary is open too broadly")
...
def _load(
    ...
    if str(prediction["schema_version"].item()) != schema:
        raise ValueError(f"invalid V13 formal source schema in {path}")
    if set(map(int, prediction["outer_fold"])) != {fold}:
        raise ValueError(f"V13 formal source fold mismatch in {path}")
    expected_seed = {-1} if seed is None else {seed}
    if set(map(int, prediction["seed"])) != expected_seed:
        raise ValueError(f"V13 formal source seed mismatch in {path}")
    return prediction
...
def score_formal(
    ...
    if assembly.get("schema_version") != ASSEMBLY_SCHEMA or assembly.get("status") != (
        "V13M4_FIVE_SEED_PREDICTION_ONLY_ASSEMBLY_PASS"
    ):
        raise ValueError("V13 formal scorer requires complete assembly")
    if merged.get("schema_version") != MERGED_SCHEMA or merged.get("status") != (
        "V13M4_COMPLETE_UNSCORED_MERGE_PASS"
    ):
        raise ValueError("V13 formal scorer requires complete fold-seed merge")
    if not merged_integrity_pass(merged.get("merge_integrity", {})):
        raise ValueError("V13 formal merged integrity is not qualified")
    if not (
        ...
        raise ValueError("V13 formal assembly violates score blindness")
    if tic2a.get("schema_version") != TIC2A_MERGED_SCHEMA or tic2a.get("status") != (
        "TIC2A_COMPLETE_UNSCORED_MERGE_PASS"
    ):
        raise ValueError("V13 formal scorer requires corrected TIC2A merge")
    ...
    if sorted(assembly_rows) != list(range(20)) or set(source_rows) != expected:
        raise ValueError("V13 formal score universes are incomplete")
    univ = M2Universe(m2_csv)
    identity = univ.build()
    if identity.get("canonical_mutant_full_profile_identity") != (
        "EXACT_PUZZLE_METHOD_MUTATION"
    ):
        raise RuntimeError("V13 formal scorer requires exact target identity")
