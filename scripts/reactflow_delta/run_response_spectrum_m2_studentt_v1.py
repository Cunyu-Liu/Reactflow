#!/usr/bin/env python3
"""run_response_spectrum_m2_studentt_v1 — robust Student-t NLL head variant of the
position-aware self-attention residual response spectrum on OpenKnot M2.

Identical protocol / exchangeable unit / residual fail-safe / CUDA-only contract as
run_response_spectrum_m2_attn_v1.py, but the loss is a per-position Student-t
negative log-likelihood (nu=4, learned per-position scale) instead of weighted-MAE.
The prediction is still prior + delta (the location), so WMAE skill and
deviation-detection remain directly comparable.

Variant name: wmae_resid_studentt_spectrum (seeds 0..4), baseline wmed_spectrum.
"""
...
import residual_spectrum_v6 as rsv6  # noqa: E402  (Student-t model)

MODEL_ID = "response_spectrum_m2_studentt_v1"
CALLER_VERSION = "m2_caller_v1"
MODEL_VARIANT = "wmae_resid_studentt_spectrum"
...
def main():
    ...
    ap.add_argument("--scale0", type=float, default=0.3)
    ...
    print(f"[m2st] GPU OK: cuda_visible={args.cuda_device} idx={gpu_index} "
    ...
    print(f"[m2st] rows={dmeta['n_rows']} designs={dmeta['n_designs']} "
    ...
    print(f"[m2st] changers with usable spectrum={len(spectra)}", flush=True)
    if not spectra:
        raise RuntimeError("no changers with usable spectra; STOP.")

    pred_path = out / "keyed_predictions_m2_studentt.jsonl"
    ...
    for fold, held in enumerate(resolved):
        ...
        pos_tr, glob_tr = rsv6.split_pos_glob(Xtr_base, WINDOW, POS_DIM)
        pos_te, glob_te = rsv6.split_pos_glob(Xte_base, WINDOW, POS_DIM)
        ...
        for seed in SEEDS:
            model, tlog = rsv6.train_posaware_student_t(
                ...
                nlayers=args.nlayers, dropout=args.dropout, nu=args.nu, scale0=args.scale0,
                seed=seed, device=device, fast=args.fast)
            delta = rsv6.predict_posaware_student_t(model, pos_te, glob_te, device)
        ...
        global_log[held] = {
            ...
            "seeds": {s: {"final_delta_abs_mean": fold_log["seeds"][s]["final"]["delta_abs_mean"],
                          ...
                          "final_scale_mean": fold_log["seeds"][s]["final"]["scale_mean"]}
    ...
    (out / "studentt_training_log.json").write_text(
        json.dumps(global_log, indent=2, sort_keys=True), encoding="utf-8")

    dhash = sha256_file(pred_path)
    manifest = {
        "schema": "reactflow_delta.response_spectrum.m2_studentt.v1",
        ...
        "experiment": "position_aware_studentt_full_spectrum_response_m2",
        ...
        "dropout": args.dropout, "nu": args.nu, "scale0": args.scale0,
        ...
        "source_hashes": {
            "run_response_spectrum_m2_studentt_v1.py": sha256_file(Path(__file__)),
            ...
            "residual_spectrum_v6.py": sha256_file(Path(__file__).resolve().parent / "residual_spectrum_v6.py"),
    ...
    (out / "response_spectrum_m2_studentt_manifest.json").write_text(
