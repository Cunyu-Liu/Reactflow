#!/usr/bin/env python3
"""Fail-closed D1-X authority preflight; performs no network or data access.

Mirrors the D0-X validator for the D1-X exact canonicalization authority
epoch.  Verifies that only D1-X is runnable, that D0-X is PASS, that the
D1-X amendment and approval hashes are bound and intact, and that the
governance Git state is clean.  No data access, no training, no split.
"""
...
def validate(root: Path, *, staging: bool = False) -> dict[str, Any]:
    ...
    check(authority.get("current_phase") == "D1-X", "current phase is not D1-X")
    check(auth.get("allowed_phases") == ["D0-X", "D1-X"], "allowed phases must equal [D0-X, D1-X]")
    check(auth.get("runnable_phases") == ["D1-X"], "runnable phases must equal [D1-X]")
    check(auth.get("full_data_recall_allowed") is True, "full recall not authorized")
    for field in (
        "training_allowed",
        "new_split_allowed",
    ...
    check(bindings.get("d1x_run_id") == "d1x_canonicalization_20260804_v1",
          "d1x_run_id mismatch")
    ...
    check(phases.get("D1-X", {}).get("execution_authorized") is True, "D1-X execution not authorized")
    ...
    return {
        "schema_version": "reactflow_delta.d1x_authority_preflight.v1",
        "status": "PASS",
