#!/usr/bin/env python3
"""Fail-closed PH0-X authority preflight; performs no network or data access.

Mirrors the D2-X validator for the PH0-X identifiability/reliability authority
epoch 7.  Verifies only PH0-X is runnable, D2-X/D1-X are PASS, the PH0-X
amendment/approval hashes are bound and intact, and the governance Git state is
clean.  No data access, no training, no blind-test inspection.
"""
...
def validate(root: Path, *, staging: bool = False) -> dict[str, Any]:
    ...
    check(authority.get("current_phase") == "PH0-X", "current phase is not PH0-X")
    check(auth.get("allowed_phases") == ["D0-X", "D1-X", "D2-X", "PH0-X"], "allowed phases mismatch")
    check(auth.get("runnable_phases") == ["PH0-X"], "runnable phases must equal [PH0-X]")
    check(auth.get("new_split_allowed") is False, "new split must be disallowed")
    for field in ("training_allowed", "confirmatory_test_access_allowed", "cross_project_export_allowed",
                  "new_wet_lab_allowed"):
    ...
    check(bindings.get("ph0x_run_id") == "ph0x_identifiability_20260804_v1", "ph0x_run_id mismatch")

    phases = {row.get("phase_id"): row for row in active.get("phase_graph", [])}
    for pid in ("RECOVERY_CONTRACT_REWRITE", "PILOT_CLOSURE", "D0-X", "D1-X", "D2-X"):
        check(phases.get(pid, {}).get("gate_result") == "PASS", f"{pid} not PASS")
    check(phases.get("PH0-X", {}).get("execution_authorized") is True, "PH0-X execution not authorized")
