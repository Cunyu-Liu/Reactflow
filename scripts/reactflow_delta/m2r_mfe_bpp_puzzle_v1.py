#!/usr/bin/env python3
"""m2r_mfe_bpp_puzzle_v1.py — puzzle-level leak-free multi-seed 3-way
+ MFE + BPP features.

Confirms the BPP partition-function feature gain at the PUZZLE level (train on
19 puzzles -> predict the held-out puzzle, with puzzle-level M2 transfer), and
compares vs the saved MFE puzzle-level multi-seed baseline.
"""
...
def run_puzzle_bpp(X, y, pz, args) -> dict:
    out = Path(args.out)
    ...
    for k, seed in enumerate(SEEDS):
        ...
        l1_seeds.append(p)
        print(f"[bpppz] L1 seed {seed} skill={_skill(_mae(y, p), mae_bl):+.4f} "
              f"wall={time.time()-t0:.0f}s", flush=True)
        p = _loo_lgb_seed(X, y, pz, puzzles, "regression", seed)
        l2_seeds.append(p)
        print(f"[bpppz] L2 seed {seed} skill={_skill(_mae(y, p), mae_bl):+.4f} "
              f"wall={time.time()-t0:.0f}s", flush=True)
    ...
    # ---- compare vs saved MFE puzzle multi-seed baseline ----
    ...
    if len(base_blend) != len(y) or np.abs(base_y - y).max() > 1e-9:
        raise SystemExit("[bpppz] baseline npz y mismatch")
    ...
    # puzzle-block permutation on the BPP multi-seed pooled skill
    ...
    report = {
        "schema": "reactflow_delta.m2r_mfe_bpp_puzzle.v1",
    ...
    (out / "m2r_mfe_bpp_puzzle_report.json").write_text(
        json.dumps(report, indent=2, sort_keys=True), encoding="utf-8")
    np.savez(out / "m2r_mfe_bpp_puzzle_oof.npz",
             blend_1=blend_1, blend_K=blend_K, y=y, puzzles=pz)
    g = report["bpp_gain_vs_mfe"]
    print(f"\n=== M2R MFE+BPP multi-seed, PUZZLE-level LOO (K={len(SEEDS)}) ===")
    print(f"MFE multi-seed    : skill={base_skill:+.4f} "
          f"R2={_r2(y, base_blend):.4f}")
    print(f"MFE+BPP multi-seed: skill={skill_K:+.4f} R2={_r2(y, blend_K):.4f}")
    ...
    print(f"wall={report['wall_seconds']:.0f}s DONE -> "
          f"{out / 'm2r_mfe_bpp_puzzle_report.json'}")
    return report
...
def main():
    ...
    ap.add_argument("--base-npz", required=True,
                    help="m2r_mfe_puzzle_oof.npz (MFE baseline)")
    ...
    print(f"[bpppz] n={len(y)} X={X.shape} puzzles={len(set(pz.tolist()))} "
          f"K={len(SEEDS)}", flush=True)

    run_puzzle_bpp(X, y, pz, args)
    return 0
