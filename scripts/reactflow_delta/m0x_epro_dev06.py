#!/usr/bin/env python3
"""EPRO_DEV_06 (M0-X, epoch-13 scope continuation): structure-aware changer
classifier.

Diagnosis of EPRO_DEV_05: a deep mechanistic EPRO backbone + changer head did
NOT converge (val study-macro AUPRC plateau ~0.52 across 60+ epochs) and did
NOT generalize (train/val gap ~0.16), far below the flat MLP changer classifier
(EPRO_DEV_04, AUPRC 0.7344) and the P2 paired baseline (0.6936).  The failure
is architectural, not capacity-driven: the deep backbone neither fits nor
generalizes while the simple per-position classifier already ranks changers
well.

EPRO_DEV_06 therefore keeps the proven flat per-position changer classifier
and injects EXPLICIT structure-aware features derived from the WT base-pair
probability (bpp) contact graph, which the 36-dim B0-X+delta_thermo vector
omits:
  * Target:  changer := |delta_true| > CHANGER_TOL * pair_scale  (binary).
  * Features (42): B0-X per-position (31) + delta_thermo (5) +
    structure-aware contact-graph (6):
      - pairing_prob   : sum_j bpp[i][j]        (prob position i is base-paired)
      - n_partners     : count of bpp partners above CONTACT threshold
      - partner_mean_unpaired      : mean WT unpaired_prob over partners
      - partner_mean_delta_unpaired: mean delta unpaired over partners
      - local_window_unpaired      : mean WT unpaired_prob in window +/-2
      - local_window_delta_unpaired: mean delta unpaired in window +/-2
  * Model:   flat MLP (proven, converges) with sigmoid head -> P(changer).
  * Loss:    focal loss (gamma>=0) with positive-class weighting.
  * Training: longer budget (hundreds of epochs) with convergence/generalization
             tracking (train vs val study-macro AUPRC gap).
  * Calibration: single-method (Platt / isotonic PAVA / temperature), fitted on
             train, reported on val (Brier / logloss / ECE) -- fixes the
             temperature-scaling numerical degradation (T->0.001) and the
             isotonic regression seen in DEV_04's cascade.
  * Baselines: published ViennaRNA Turner-rules physics (|Delta(unpaired)|),
             P2 paired, wt_only -- same changer-detection task.
...
required (fallback=0).  No pretraining, so no exposure-audit entry; test is
never read.
"""
...
from m0x_calibration import fit_and_report, platt_calibrate, isotonic_calibrate, \
    temperature_calibrate, brier_logloss, ece  # noqa: E402

# --- Frozen constants (seed + dev defs identical to prior EPRO_DEV iterations) ---
SEED = 20260804
TEMPERATURE = 37.0
CHANGER_TOL = 0.05
...
# ---------------------------------------------------------------------------
# Feature pipeline: B0-X features + delta_thermo + structure-aware contact-graph
# ---------------------------------------------------------------------------
...
def _structure_aware_features(pair, wt_cache, delta_thermo) -> np.ndarray:
    """Per-position (n) x 6 structure-aware contact-graph features.

    Uses the WT base-pair probability (bpp) matrix to encode pairing context
    and structural partner aggregation that the per-position B0-X vector omits.
    """
    wt = wt_cache.get(pair.pair_id)
    if wt is None:
        wt = compute_wt_thermo_state(pair.seq, temperature=TEMPERATURE)
        wt_cache[pair.pair_id] = wt
    n = len(pair.mask)
    bpp = np.asarray(wt["bpp"], dtype=np.float64)          # (n_seq, n_seq)
    ...
    # Local structural context windows (+/-2).
...
def _build_pair_records(pairs: list, wt_cache, mut_cache) -> list[dict]:
    """Per-pair record: features (n,42), changer labels (n,), mask, study, parent.

    Features = B0-X (31) + delta_thermo (5) + structure-aware contact-graph (6).
    """
    recs = []
    for p in pairs:
...
# ---------------------------------------------------------------------------
# Training (with convergence/generalization tracking)
# ---------------------------------------------------------------------------
def _train_classifier(model, X, y, val_recs, device, epochs, batch_size, lr,
    ...
    for epoch in range(epochs):
        ...
        if (epoch + 1) % eval_every == 0 or epoch == epochs - 1:
            ...
            val_auprc = _study_macro_auprc(changed)
            ...
            print(f"[train] epoch {epoch+1}/{epochs} loss={epoch_loss:.4f} "
                  f"val_auprc={val_auprc:.4f} best={best_auprc:.4f}@{best_epoch} "
...
# Published baselines
# ---------------------------------------------------------------------------
def _vienna_physics_scores(recs, wt_cache) -> dict[str, np.ndarray]:
    """Published ViennaRNA (Turner-rules) physics changer score.

    |Delta(unpaired_prob)| per position from in-silico WT->mutant mutagenesis,
    served directly as the changer score (no training).
    """
    out = {}
    for r in recs:
        out[r["pair_id"]] = np.abs(r["delta_thermo"][:, 0]).astype(np.float32)
    return out
...
def main() -> int:
    ...
    print(f"[model] StructureAwareChangerClassifier params={model.param_count():,}",
          flush=True)
    ...
    wt_changed = _changer_records(val_recs, {p.pair_id: np.abs(wt_ref_preds[p.pair_id])
                                              for p in val})
    vienna_changed = _changer_records(val_recs, _vienna_physics_scores(val_recs, wt_cache))
    ...
    vienna_auprc = _study_macro_auprc(vienna_changed)
    ...
    # Calibrated AUPRC (re-score with calibrated probs).
    cls_val_cal = []
    k = 0
    ...
    # AUPRC gain + CI vs POLA (structure-aware), P2, ViennaRNA, wt.
    gain_vs_p2 = _auprc_gain_bootstrap(cls_changed, p2_changed,
                                       n_boot=args.n_boot, seed=SEED)
    gain_vs_vienna = _auprc_gain_bootstrap(cls_changed, vienna_changed,
                                           n_boot=args.n_boot, seed=SEED)
    gain_vs_wt = _auprc_gain_bootstrap(cls_changed, wt_changed,
                                       n_boot=args.n_boot, seed=SEED)

    # PASS: structure-aware classifier AUPRC-gain cluster CI low > 0 vs P2.
    ci_low = gain_vs_p2["ci_low"]
    pass_ok = (not math.isnan(ci_low)) and ci_low > 0

    table = {
        "structure_aware_changer": {
        ...
        "vienna_physics_published": {"param_count": 0,
                                     "study_macro_auprc": vienna_auprc,
    ...
    manifest = {
        ...
        "evaluation": {
            "structure_aware_changer": {"study_macro_auprc": cls_auprc,
                                        "train_study_macro_auprc": tr_auprc,
                                        "gen_gap": gen_gap,
                                        "study_macro_auprc_calibrated": cls_cal_auprc,
            ...
            "vienna_physics_published": {"study_macro_auprc": vienna_auprc},
        },
        "comparison_table": table,
        "pass": {"pass": pass_ok, "auprc_gain_ci_low_vs_p2": ci_low,
                 "note": "pass = structure-aware classifier AUPRC-gain cluster CI low > 0 vs P2 baseline"},
    }
    ...
    np.savez_compressed(str(out_dir / "predictions.npz"),
                        structure_aware=dict(val_score),
                        vienna=dict(_vienna_physics_scores(val_recs, wt_cache)),
                        p2=dict(p2_val_preds), wt=dict(wt_ref_preds))

    print("\n=== COMPARISON TABLE ===", flush=True)
    print(f"structure_aware params={model.param_count():,} "
          f"auprc={cls_auprc:.4f} train={tr_auprc:.4f} gap={gen_gap:.4f}"
          f" cal={cls_cal_auprc:.4f}", flush=True)
    print(f"vienna_physics(P) params=0 auprc={vienna_auprc:.4f}", flush=True)
