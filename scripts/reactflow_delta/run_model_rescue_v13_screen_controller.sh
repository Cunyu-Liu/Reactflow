...
repo=/home/cunyuliu/reactflow_delta_worktrees/model_rescue_v13_20260826
...
out=/mnt/cunyuliu/reactflow_delta_model_rescue_v13/v13m3_screen_seed0
...
run_worker() {
  ...
  for fold in "${requested[@]}"; do
    if [[ ! -f "${out}/v13_fold_result_fold${fold}_seed0.json" ]]; then
      missing+=("${fold}")
    fi
  done
  ...
  CUDA_VISIBLE_DEVICES="${gpu}" "${python_bin}" -m \
    scripts.reactflow_delta.run_model_rescue_v13 \
      --repo-root "${repo}" \
      --phase V13M3 \
      --m2-csv "${m2}" \
      --v8-dir "${v8_dir}" \
...
if [[ "${failed}" -ne 0 ]]; then
  printf 'one or more V13M3 prediction workers failed\n' >&2
  exit 1
fi

"${python_bin}" -m scripts.reactflow_delta.merge_model_rescue_v13 \
  --input-dir "${out}" \
  --phase V13M3 \
  --out-json "${out}/v13m3_complete_unscored_merge.json"
touch "${out}/v13m3_complete_prediction_only_merge_pass"
