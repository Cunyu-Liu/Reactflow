#!/usr/bin/env python3
"""m2r_permtest_v2.py — fast permutation test + bootstrap CI for the M2R GBDT.

The slow part of the earlier attempt was re-training the GBDT inside the
permutation loop.  This script trains the default GBDT LOO ONCE, then does the
permutation / bootstrap on the OOF predictions (which is the correct
design-block procedure and needs no re-training).
"""
...
def main():
    ...
    # save OOF preds + y + keys for reuse
    out = Path(args.out)
    out.mkdir(parents=True, exist_ok=True)
    np.savez(out / "m2r_gbdt_oof.npz",
             preds=gbdt_preds, y=y, keys=keys)

    # ---- design-block permutation test (on OOF preds) ----
    ...
    report = {
        "schema": "reactflow_delta.m2r_permtest.v2",
