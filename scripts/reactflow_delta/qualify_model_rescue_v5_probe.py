#!/usr/bin/env python3
"""Apply the pre-frozen V5M2 structure-delta eligibility Gate."""
...
SCHEMA = "reactflow_delta.model_rescue_v5_probe_qualification.v1"


def paired_summary(effects: list[float]) -> dict[str, Any]:
    values = np.asarray(effects, dtype=np.float64)
    if values.shape != (20,):
        raise ValueError("V5M2 qualification requires exactly 20 puzzle effects")
...
def qualify(scores: dict[str, Any]) -> dict[str, Any]:
    ...
    if scores.get("status") != "V5M2_COMPLETE_SCORE_PASS" or len(rows) != 20:
        raise ValueError("v5 qualifier requires one complete V5M2 score artifact")
    rows = sorted(rows, key=lambda row: int(row["outer_fold"]))
    if [int(row["outer_fold"]) for row in rows] != list(range(20)):
        raise ValueError("v5 qualifier requires unique outer folds 0 through 19")
    ...
    return {
        ...
        "phase": "V5M2",
        "overall_status": (
            "V5M2_STRUCTURE_DELTA_SIGNAL_ELIGIBLE"
            if passed
            else "MODEL_RESCUE_V5_FAIL"
        ),
        "v5m3_authorized": passed,
