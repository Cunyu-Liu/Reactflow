#!/usr/bin/env python3
"""Join corrected targets only after the complete V7M2 prediction merge."""
...
from scripts.reactflow_delta.merge_model_rescue_v7_probe import SCHEMA as MERGED_SCHEMA
...
SCHEMA = "reactflow_delta.model_rescue_v7_probe_score.v1"
...
def assert_score_authority(repo_root: Path) -> None:
    ...
    if active["authority"]["current_phase"] != "V7M2":
        raise RuntimeError("V7M2 scorer is closed outside active V7M2")
    if active.get("held_score_read_allowed") is not True:
        raise RuntimeError("complete V7M2 score access is closed")
    if active.get("partial_fold_score_read_allowed") is not False:
        raise RuntimeError("partial V7M2 scores must remain closed")
    if active.get("new_external_outcome_access_allowed") is not False:
        raise RuntimeError("V7M2 scorer requires external outcomes locked")
...
def _load_prediction(path: Path, outer_fold: int) -> dict[str, np.ndarray]:
    ...
    if not np.array_equal(keys, prediction["biological_scoring_key"]):
        raise ValueError(f"V7M2 prediction keys disagree in {path}")
    if len(set(map(str, keys))) != len(keys):
        raise ValueError(f"V7M2 prediction keys are duplicated in {path}")
    if set(map(int, prediction["outer_fold"])) != {outer_fold}:
        raise ValueError(f"V7M2 prediction outer fold differs in {path}")
    for field in PREDICTION_FIELDS:
        if prediction[field].shape != (len(keys),) or not np.isfinite(
            prediction[field]
        ).all():
            raise ValueError(f"V7M2 prediction {field} is invalid in {path}")
    forbidden = {
        "target",
        "target_error",
        "target_mask",
...
def score_fold(
    ...
    for record in held_records:
        ...
        for position in np.flatnonzero(qualified):
            ...
            if key not in key_to_index:
                raise ValueError("qualified V7M2 target is missing from prediction")
    ...
    result = {f"{field}_mae": _puzzle_macro(values) for field, values in losses.items()}
    result.update(
        {
...
def score_complete_merged(merged: dict[str, Any], m2_csv: Path) -> dict[str, Any]:
    ...
    if ledger.get("n_canonical_mutant_full_profiles") != 13976:
        raise RuntimeError("V7M2 scorer requires 13,976 canonical target profiles")
