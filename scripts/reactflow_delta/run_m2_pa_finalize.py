#!/usr/bin/env python3
"""run_m2_pa_finalize.py — once the M2 position-aware 159-fold run completes, run
the FULL deviation-detection (heavy permutation) and regenerate the integrated
method summary (WMAE skill + deviation-detection) with full-data numbers.

The running run_m2_posaware_compare.py already produces the full WMAE
m2_pa_full_comparison.json at 159 folds.  This driver fills the remaining gap:
  * full PA deviation-detection report (m2_deviation_report.py, 300 perms, ~2h)
  * regenerated m2_pa_method_summary.json with PA full vs MLP full reference

Usage (A100 server, editflow env):
    python run_m2_pa_finalize.py \
...
def main():
    ...
    print(f"[pa_finalize] waiting for {args.total}-fold PA run...", flush=True)
    while time.time() < deadline:
        n = _progress(args.progress)
        print(f"[pa_finalize] completed {n}/{args.total}", flush=True)
        if n >= args.total:
            break
        time.sleep(args.poll_secs)
    if _progress(args.progress) < args.total:
        print("[pa_finalize] TIMEOUT waiting for full run", flush=True)
        return 1

    # 1. full deviation-detection (heavy permutation)
    dev_out = out / "deviation_pa_full"
    dev_out.mkdir(parents=True, exist_ok=True)
    cmd = [sys.executable, str(script / "m2_deviation_report.py"),
           "--pred", args.pred_pa, "--out", str(dev_out),
           "--model-variant", PA_VARIANT, "--n-perm", str(args.n_perm)]
    print(f"[pa_finalize] running full deviation... {' '.join(cmd)}", flush=True)
    subprocess.run(cmd, check=True)
    pa_dev = json.loads((dev_out / "m2_deviation_report.json").read_text(encoding="utf-8"))

    # 2. full WMAE horizontal (in case the compare driver's output is not yet there)
    pa_h_rep = out / "m2_horizontal_ensemble_report_pa_full.json"
    cmd = [sys.executable, str(script / "m2_horizontal_ensemble_report.py"),
           "--pred", args.pred_pa, "--out", str(out),
           "--model-variant", PA_VARIANT]
    print(f"[pa_finalize] running full WMAE horizontal...", flush=True)
    subprocess.run(cmd, check=True)
    (out / "m2_horizontal_ensemble_report.json").replace(pa_h_rep)
    pa_h = json.loads(pa_h_rep.read_text(encoding="utf-8"))
    ...
    summary = {
        "schema": "reactflow_delta.response_spectrum.m2_pa_method_summary.v2",
        ...
        "deviation_detection": {
            ...
            "position_aware": _drow(pa_dev),
        },
    }
    (out / "m2_pa_method_summary_full.json").write_text(
        json.dumps(summary, indent=2, sort_keys=True), encoding="utf-8")
    print("\n=== M2 position-aware FULL (159-fold) method summary ===")
    print(json.dumps(summary, indent=1, ensure_ascii=False))
    print(f"\nDONE -> {out / 'm2_pa_method_summary_full.json'}")
    return 0
