...
python_bin=${REACTFLOW_V8_PYTHON:-/home/cunyuliu/miniconda3/envs/editflow/bin/python}
...
"${python_bin}" -m scripts.reactflow_delta.merge_model_rescue_v8_mean_screen \
  --input-dir "${v8_dir}" \
  --qualification-json "${v8_dir}/v8m1_corrected_expert_qualification.json" \
  --out-json "${v8_dir}/v8m2_complete_unscored_merge.json"

"${python_bin}" -m scripts.reactflow_delta.score_model_rescue_v8_mean_screen \
  --repo-root "${repo_root}" \
...
"${python_bin}" -m scripts.reactflow_delta.qualify_model_rescue_v8_mean_screen \
