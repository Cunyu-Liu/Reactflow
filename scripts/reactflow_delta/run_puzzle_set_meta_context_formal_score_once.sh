...
score=${out}/p1m4_complete_formal_score.json
qualification=${out}/p1m4_formal_qualification.json
screen_qualification=/mnt/cunyuliu/reactflow_delta_puzzle_set_meta_context/p1m3_screen_seed0/p1m3_qualification.json
...
cd "${repo}"
for required in \
  "${merged}" "${assembly}" "${screen_qualification}" \
  "${m2}" "${tic2a}" "${v13_score}"; do
  if [[ ! -f "${required}" ]]; then
    printf 'required frozen puzzle-set formal input is missing: %s\n' \
      "${required}" >&2
    exit 1
  fi
done
if [[ -e "${score}" || -e "${qualification}" ]]; then
  printf 'puzzle-set formal score-once output already exists; refusing to rerun\n' >&2
  exit 1
fi

"${python_bin}" -m scripts.reactflow_delta.score_puzzle_set_meta_context_formal \
  --repo-root "${repo}" \
...
"${python_bin}" -m scripts.reactflow_delta.qualify_puzzle_set_meta_context_formal \
  --score-json "${score}" \
