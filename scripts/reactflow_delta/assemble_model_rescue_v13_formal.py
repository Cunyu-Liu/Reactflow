#!/usr/bin/env python3
"""Assemble V13 five-seed equal mixtures without scoring."""
...
SCHEMA = "reactflow_delta.model_rescue_v13_formal_assembly.v1"
FORMAL_PREDICTION_SCHEMA = "reactflow_delta.model_rescue_v13_formal_prediction.v1"
...
def _load_prediction(path: Path, fold: int, seed: int) -> dict[str, np.ndarray]:
    ...
    if str(prediction["schema_version"].item()) != PREDICTION_SCHEMA:
        raise ValueError(f"invalid V13 source schema in {path}")
    if set(map(int, prediction["outer_fold"])) != {fold} or set(
        map(int, prediction["seed"])
    ) != {seed}:
        raise ValueError(f"V13 formal source fold/seed mismatch in {path}")
    keys = list(map(str, prediction["keys"]))
    if len(keys) != len(set(keys)):
        raise ValueError(f"duplicate V13 formal source keys in {path}")
    return prediction
...
def assemble_fold(
    ...
    if sorted(by_seed) != list(range(5)):
        raise ValueError(f"V13 formal fold {fold} requires seeds0-4")
    source = [
        _load_prediction(Path(by_seed[seed]["prediction_artifact"]), fold, seed)
        for seed in range(5)
    ]
    keys = list(map(str, source[0]["keys"]))
    for seed, prediction in enumerate(source[1:], start=1):
        if list(map(str, prediction["keys"])) != keys:
            raise ValueError(f"V13 fold {fold} seed{seed} key order differs")
        if not np.allclose(
            prediction["feature41_point"], source[0]["feature41_point"], atol=1e-7, rtol=0.0
        ):
            raise ValueError(f"V13 fold {fold} feature41 differs by seed")
    n_rows = len(keys)
    output: dict[str, np.ndarray] = {
        ...
        "candidate_point": np.mean(
            [np.asarray(item["candidate_point"], dtype=np.float64) for item in source], axis=0
        ),
        "null_point": np.mean(
            [np.asarray(item["null_point"], dtype=np.float64) for item in source], axis=0
        ),
    }
    for name in POINT_NAMES:
        ...
        if weights.shape != (n_rows, 10) or locations.shape != (n_rows, 10) or scales.shape != (n_rows, 10):
            raise ValueError("V13 formal distributions must contain ten components")
        if not np.allclose(weights.sum(axis=1), 1.0, atol=1e-7, rtol=0.0):
            raise ValueError("V13 formal seed weights do not sum to one")
        if not np.isfinite(locations).all() or not (scales > 0.0).all():
            raise ValueError("V13 formal mixture contains an invalid component")
    ...
    path = out_dir / f"v13_formal_predictions_fold{fold}_seeds0_4.npz"
...
def assemble(merged: dict[str, Any], out_dir: Path) -> dict[str, Any]:
    if merged.get("schema_version") != MERGED_SCHEMA or merged.get("status") != (
        ...
        raise ValueError("V13 formal assembler requires complete V13M4 merge")
    ...
    if any(len(rows) != 5 for rows in rows_by_fold.values()):
        raise ValueError("V13 formal assembler requires five runs per fold")
    ...
    return {
        ...
        "phase": "V13M4",
        "status": "V13M4_FIVE_SEED_PREDICTION_ONLY_ASSEMBLY_PASS",
        "folds": folds,
