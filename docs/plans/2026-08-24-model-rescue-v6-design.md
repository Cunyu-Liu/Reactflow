...
The V6M2 baseline is the fixed v5 ridge feature set: 18 geometric/WT-context covariates plus the 12 unconstrained ensemble-delta features. An outcome-blind algebraic audit performed before any v6 score access identified the exact identity `delta_unpaired + delta_upstream_mass + delta_downstream_mass = 0`. The v6 probe therefore adds the information-equivalent 11-dimensional constrained basis, excluding downstream mass because it is exactly reconstructible. It uses train-only weighted standardization and ridge alpha 1, with position → mutant → puzzle-method cell weighting. A complete 20-fold prediction universe must exist before target join.

Only an exact eligibility PASS opens neural work. The primary model then trains corrected B1 identically, freezes it, and trains a zero-initialized residual head on detached B1 source-receiver features plus 22 independent structure features. Two mandatory, non-selectable controls use the same head dimensions and budget: a 22-channel zero-structure capacity null and an unconstrained-only 11-channel control padded with eleven zero channels. The primary receives the 11-dimensional unconstrained and constrained bases. This makes a primary-versus-control difference attributable to experimental constraint information rather than head capacity or an exact duplicate channel.
