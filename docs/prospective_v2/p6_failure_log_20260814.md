...
| F8 | P5b | 预冻结负对照在 NEW 独立组件（M2RFOK/M2RFPK，505 可评估）上未通过（permuted CI upper +0.0204 > 0） | 模型对 readout WT reactivity（coef 0.62）的收缩效应在 BigLib2 上无法被位点置换完全消除（极端位点向均值收缩仍优于 raw WT）；Ribonanza 集则 clean（CI upper −0.062） | 按冻结协议 fail-closed：MECHANISM_NOT_ESTABLISHED；主 claim（远距离空间扩展）仍强确认（vfar CI lower +0.0835，p<1e-89），但 feature-dependence 控制不 clean | 诚实报告；不改冻结 gate，不回流调参 | `test_p5b_mechanism_v1.py`（11 PASS） |
